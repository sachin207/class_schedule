...
$query = "SELECT first_name,last_name,username,email,dept_id,cabin_no,phone FROM users WHERE id ='$user_id'";
...
if (isset($_POST['submit'])) {
    ...
    if (empty($form_errors)) {
      ...
      if (mysqli_num_rows($data) == 1) {
        $row = mysqli_fetch_array($data);
        $query = "UPDATE users SET first_name = '$first_name', last_name = '$last_name', username = '$username', email = '$email', dept_id = '$row[dept_id]', cabin_no = '$cabin_no', phone = '$phone' WHERE id ='$user_id'";
        $result = mysqli_query($dbc, $query);
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                    ...
                    <input type="submit" value="Edit Profile" id="submit" name="submit" class="btn btn-success "/>
                </form>
            </div>                           
        </div>
    </div>
